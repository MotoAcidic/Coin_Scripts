...
echo Downloading smrtc install files.
wget https://github.com/SMRT-Cloud/smrtc/releases/download/v1.0.0.0/smrtc-linux.tar.gz
echo Download complete.
echo Installing smrtc.
tar xvf smrtc-linux.tar.gz
chmod 775 ./smrtcd
chmod 775 ./smrtc-cli
echo smrtc install complete. 

echo Now ready to setup smrtc configuration file.
...
mkdir -p /root/.smrtc && touch /root/.smrtc/smrtc.conf

cat > /root/.smrtc/smrtc.conf << EOF
rpcuser=$RPCUSER
...
echo Please start your new smrtc masternode by running ./smrtcd -daemon
echo If you get a message asking to rebuild the database, please hit Ctr + C and run ./smrtcd -daemon -reindex
...
echo smrtc configuration file has been updated successfully.
echo You can start your new smrtc masternode manually by running ./smrtcd -testnet
echo If you get a message asking to rebuild the database, please hit Ctr + C and run ./smrtcd -daemon -reindex
echo If you still have further issues please reach out to support in our Discord channel.
            ;;
        3)
