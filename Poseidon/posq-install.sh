...
WIDTH=40
CHOICE_HEIGHT=6
BACKTITLE="Poseidon Masternode Setup Wizard"
TITLE="Poseidon VPS Setup"
MENU="Choose one of the following options:"

OPTIONS=(1 "Install New VPS Server"
         2 "Update to new version VPS Server"
         3 "Start Posedion Masternode"
	 4 "Stop Poeseidon Masternode"
	 5 "Poseidon Server Status"
	 6 "Rebuild Poseidon Masternode Index")
...
echo Now ready to setup Poseidon configuration file.
...
mkdir -p /root/.poseidon && touch /root/.poseidon/poseidon.conf

cat << EOF > /root/.poseidon/poseidon.conf
rpcuser=$RPCUSER
...
addnode=seed.posq.io
addnode=seed.posq.space
EOF
clear
...
./poseidon -daemon
clear
echo Poseidon configuration file created successfully. 
echo Poseidon Server Started Successfully using the command ./poseidond -daemon
...
echo Please use the following Private Key when setting up your wallet: $GENKEY
        ...
        2)
sudo ./poseidon-cli -daemon stop
echo "! Stopping Poseidon Daemon !"
